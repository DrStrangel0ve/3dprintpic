...
The webapp includes printer-volume constraints for STL sizing. The default preset is `Bambu Lab P1S` with a `256 x 256 x 256 mm` build volume; custom printer dimensions, print scale, and edge clearance can be set in the output panel. For 2.5D relief STL generation, `max_xy_size` is the requested output footprint, `detail_basis_mm` is the full usable footprint on which filtering and geometry guards run, `target_dimension` is the sample budget, and `z_scale` is the clamped relief height. The approved height field is calculated once at the full-size detail basis and only its final X/Y coordinates are scaled for a smaller print. A 30% export therefore retains the same Z samples as a 100% export instead of being resampled according to its smaller millimeter footprint. `mesh_sample_pitch_mm` continues to mean the emitted STL pitch, while `processing_mesh_sample_pitch_mm` records the full-size calculation pitch. `emitted_printability` audits post-scale slopes and scaled feature width explicitly: retaining recognition at a very small footprint can preserve geometry finer or steeper than the configured nozzle can reproduce, and the metadata does not overstate that physical limit. See [Scale-independent relief sampling](docs/relief-scale-independent-sampling.md).

The default relief polarity is `raised-print`, and the backend chooses the effective invert setting from the actual depth model semantics so near-high relative depth models and far-high distance models both make faces/subjects protrude; switch to `Mold` only when negative relief is intentional. Metric/distance outputs such as Apple Depth Pro are preserved without destructive min-max storage and converted to inverse-depth proximity before relief normalization, which prevents distant sky pixels from flattening the subject into a near-constant-height slab. These runs keep `output_depth_preview.png` as the raw depth diagnostic and add `output_relief_preview.png` for the transformed height signal. The relief writer also performs robust percentile normalization, local feature boosting, a gamma relief curve, lower smoothing, and an optional crisp border ring so small features such as noses and sharper rear/side walls survive the depth-to-STL conversion. Each relief job writes `output_model.stl`, `diagnostics.json`, and `metadata.json`; the response includes `diagnostics_url` and inline STL checks for watertightness, manifoldness, positive volume, connected body count, bbox health, and mesh complexity.

`Trim empty sky` fuses structural image boundaries with sustained depth evidence instead of treating every cloud edge as geometry. SAM-selected pixels are a hard preservation constraint, alpha remains authoritative for transparent cutouts, and ambiguous depth falls back to the structural or full-rectangle path. The crop is applied only when emitting the mesh, so every retained face, roof, tower, and background height sample is bit-for-bit identical to an untrimmed run.
...
RELIEF_BACKEND_URL=http://127.0.0.1:8014
MASV_API_KEY=
MASV_TEAM_ID=
RBC_ACCESS_TOKEN=
```
...
- Production depth: `Depth Anything V2 Large` is the one model exposed by the product because it remains the best verified CUDA-backed relief-depth option. The August 2026 audit tested DA3MONO, InfiniDepth, and MetricAnything PointMap/DepthMap; every challenger regressed 30 mm facial geometry and none passed the background gate. The pinned DA2 model-card weights are CC BY-NC 4.0, so commercial deployment needs separate permission or a future permissively licensed candidate that passes the same gates. See `docs/benchmark-evidence/sota_depth_challengers_20260810`.
- Depth semantics: far-high metric/distance models are converted through inverse depth in benchmark tooling; the production relative near-high model retains linear shaping. Exact semantics and model revisions are recorded with every run.
- Image completion and inpainting are not production features. The application preserves original source pixels; legacy completion providers and training runs remain research history only.
