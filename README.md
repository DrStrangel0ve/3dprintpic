...
`Trim empty sky` uses a structural skyline boundary rather than comparing the frame with one sampled sky color. This removes smooth clouds, night gradients, and other low-information space above the first mapped feature while preserving rooflines, hair, towers, and transparent cutout boundaries. Ambiguous frames with no trustworthy structural boundary fail closed and retain the complete rectangular relief.
