...
- STL-first mesh path: the benchmark can now compare direct image-to-mesh candidates against the depth-relief STL route. `source-mesh-oracle` is a ground-truth diagnostic ceiling for rendered mesh datasets, and `external-image-to-mesh` can wrap modern single-image mesh generators such as TripoSR, Stable Fast 3D, SPAR3D, or Hunyuan3D, then score the emitted STL for mesh-surface error, watertightness, body count, bounding-box health, and printable complexity.
