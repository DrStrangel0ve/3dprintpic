...
Measured STL-first update on July 10, 2026: Colab G4 run `g4_stl_first_triposg_bbox_tuning_s40_n10` completed all `70/70` rows (`7` methods x `10` samples) on an RTX PRO 6000 Blackwell with 95 GB VRAM. The TripoSG biharmonic-prefill candidate calibrated to deployable depth-relief bbox extents scored `0.9280740` versus `0.1991798` for `mirror`, with median mesh Chamfer `0.1417979`; its decision is still `hold` solely because `5/10` meshes exceeded the per-sample scale-free complexity limit. The next generated run config is `backend/benchmark/experiment_configs/modelnet10_60_balanced_stl_quality_triposg_inferred_bbox_generated.json`, configured with the locally validated `--mesh-max-normalized-face-density-log1p 9.95` path so decimation follows the exact printability metric rather than a fixed global face target. See the benchmark doc for checksums, outliers, and the diagnostic source-oracle result.
