...
Current branch refresh for the next G4 rerun: `modelnet10_heldout10_stl_first_triposg_bbox_tuning_colab_inputs.tar.gz` is regenerated from the `codex/3d-completion-benchmark-g4` branch after the scale-free STL-complexity scoring update, with `--require-image-to-mesh-providers`, `--colab-require-gpu-name-regex "RTX PRO 6000|Blackwell"`, and `--colab-min-gpu-memory-gb 90`. The generated `run_colab_eval.sh` writes `gpu_preflight.json` first and aborts before archive extraction, repo sync, or provider setup if Colab attaches CPU/T4/L4/A100-class hardware instead of the intended G4/Blackwell runtime. It still writes `image_to_mesh_provider_preflight.json` after setup and fails before rows if `/content/TripoSG`, `/content/triposg-venv/bin/python`, or the TripoSG entrypoint is missing. The branch-hosted archive is `1,624,854` bytes with SHA256 `a692c6fbcc2487cfc1068a49c25772d58eb93ed0e91e209844206d5513890bbd`; the branch-hosted fetch notebook is `5,794` bytes with SHA256 `3af79b1f197ca4dcd2c9a8aad85e3abc61f24df5696d32d6b50025a73a4f62d1`; and the fetch cell is `3,897` bytes with SHA256 `74369e886c162b612b1b5d66004aaca2f654e62913ea76ce8341f6935fd0cdbc`. Open `https://colab.research.google.com/github/DrStrangel0ve/3dprintpic/blob/codex/3d-completion-benchmark-g4/colab_payloads/run_colab_triposg_bbox_tuning_eval_fetch_notebook.ipynb` on an actual G4 runtime for the next promotion-quality attempt.
