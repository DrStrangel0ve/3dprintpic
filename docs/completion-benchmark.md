...
Second setup-logged Hunyuan launch note: the `aa8d4add...` notebook produced the compact result archive and showed `run_status=1`; the captured log traced the failure to `backend/pic_to_3d.py` importing `from stl import mesh` before `numpy-stl` was installed. `numpy-stl` already lives in `backend/requirements.txt`, so the fix is to install backend requirements in `run_colab_eval.sh` immediately after syncing the repo and before Hunyuan provider setup or prefetch. The next regenerated notebook should validate that this clears the `No module named 'stl'` failure before evaluating Hunyuan quality.
