...
The rank-score explainer now decomposes `rank_score` into metric contributions and writes `rank_score_explanation.md/json/csv` beside future `optimize_completion` runs. Running it on the prefill Colab aggregate shows the actual tradeoff: repaired TripoSR variants gain `+0.58` to `+0.69` from mesh-surface Chamfer/RMSE/H95 improvements, but lose `-2.18` to `-2.30` from `stl_faces_per_bbox_volume_log1p_median` and `-0.32` to `-0.77` from `stl_bbox_aspect_ratio_median`. The next Colab slice should rerun the same held-out rows with `triposr_api_*_repaired_stl_scaled_compact_direct_mesh` candidates, which apply `--mesh-target-max-dimension 96 --mesh-min-bbox-dimension 12` after printable repair and before STL export. If these variants keep the mesh-surface gains while removing the face-density/aspect penalties, the next optimization target becomes remeshing/decimation; if not, the bottleneck is the direct mesh shape itself rather than STL scale.
