...
If the Colab UI has trouble accepting the 2 MB inline paste, use the smaller fetch cell `run_colab_hunyuan3d_shape_eval_fetch_cell.py` from the local `outputs` directory or open the paste-free Colab notebook at `https://colab.research.google.com/github/DrStrangel0ve/3dprintpic/blob/9e6347fbea40ff4400b54112aa31afb95c122b70/colab_payloads/run_colab_hunyuan3d_shape_eval_fetch_notebook.ipynb`. The notebook is published from payload commit `9e6347fbea40ff4400b54112aa31afb95c122b70`, has raw SHA256 `3260b647bf0863b33a67ee5d929d8381481cbd48c7e9432bbe9f442ccfe16213`, and downloads the immutable tarball from archive commit `a1ccd32f5eeaea603e209323b103ec013758bc53`. The tarball raw download was validated at `1,621,895` bytes with SHA256 `11a0da1843e362772d7e395258d737df22fe777cf109fce0e43c0782f9ff732f`; the fetch launcher verifies that exact size/SHA, extracts `run_colab_eval.sh`, and launches the same Hunyuan setup/eval path.

First setup-logged Hunyuan launch note: the previous fetch notebook (`0276b6df...`) verified the `238125c...` archive and entered `run_colab_eval.sh`, but the script exited with status `1` before writing `launch_preflight.json`, `run_colab_eval.log`, `results_summary.json`, or `/content/g4_stl_first_hunyuan3d_shape_s40_n10_results.tar.gz`. The current `aa8d4add...` notebook uses a regenerated launcher that tees the entire setup stage, not only the orchestrator stage, and writes the compact result archive even when provider setup fails early.

Second setup-logged Hunyuan launch note: the `aa8d4add...` notebook produced the compact result archive and showed `run_status=1`; the captured log traced the failure to `backend/pic_to_3d.py` importing `from stl import mesh` before `numpy-stl` was installed. `numpy-stl` already lives in `backend/requirements.txt`, so the fix is to include the orchestrator `setup` stage in generated Colab package launchers. The current `9e6347f...` notebook includes that setup stage before the Hunyuan provider setup and benchmark eval.
...
Colab G4 source-bbox oracle calibration result: the notebook terminal ran commit `e3b0941` as `g4_stl_first_triposr_source_bbox_s40_n10`, using the same packaged held-out manifest and isolated TripoSR API venv. It completed successfully, wrote `/content/g4_stl_first_triposr_source_bbox_s40_n10_results.tar.gz` (`26,375,237` bytes), and the selector decision was `promote` for the oracle-calibrated candidate:
