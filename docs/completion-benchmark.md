...
Provider priority for the current G4 lane is Step1X-3D geometry-only as the new upstream challenger and the promoted TripoSG biharmonic-prefill path as the fixed control. Step1X is pinned to public Apache-2.0 source and model snapshots, emits a native TSDF-derived mesh, and has a dedicated CUDA 12.8/Blackwell preflight. Its one-row config keeps raw output diagnostic-only and applies the shared printable repair only to a separate candidate row. SPAR3D remains install-blocked on the current Python 3.12 image, while Hunyuan3D-2mv and TRELLIS.2 post-hoc repair lanes are closed by measured drift/topology failures. SV3D/VGGT/COLMAP/OpenMVS-style methods remain in the `external-multiview-to-mesh` lane because they synthesize views, cameras, depths, or point clouds before any final STL-producing mesh stage.
