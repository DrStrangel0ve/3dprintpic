...
Current branch refresh for the next G4 rerun: `modelnet10_heldout10_stl_first_triposg_bbox_tuning_colab_inputs.tar.gz` is regenerated from the `codex/3d-completion-benchmark-g4` branch with `--require-image-to-mesh-providers`, `--colab-require-gpu-name-regex "RTX PRO 6000|Blackwell"`, and `--colab-min-gpu-memory-gb 90`. The generated `run_colab_eval.sh` writes `gpu_preflight.json` first and aborts before archive extraction, repo sync, or provider setup if Colab attaches CPU/T4/L4/A100-class hardware instead of the intended G4/Blackwell runtime. It still writes `image_to_mesh_provider_preflight.json` after setup and fails before rows if `/content/TripoSG`, `/content/triposg-venv/bin/python`, or the TripoSG entrypoint is missing. The branch-hosted archive is `1,624,842` bytes with SHA256 `f39681baa940a97153aceabfe2215d2971a2f528d7e8036170628c140bfef77f`; the branch-hosted fetch notebook is `5,794` bytes with SHA256 `b17baf1890da1d5d3930dee02c3e08f0546e618bd071003610e508a81cba2713`; and the fetch cell is `3,897` bytes with SHA256 `2fbd916b8e27b1c6fe19d637e8516a539ba04ff60c90b84c7ddd05a0df2a620f`. Open `https://colab.research.google.com/github/DrStrangel0ve/3dprintpic/blob/codex/3d-completion-benchmark-g4/colab_payloads/run_colab_triposg_bbox_tuning_eval_fetch_notebook.ipynb` on an actual G4 runtime for the next promotion-quality attempt.
