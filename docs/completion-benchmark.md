...
Current branch refresh for the next G4 rerun: `modelnet10_heldout10_stl_first_triposg_bbox_tuning_colab_inputs.tar.gz` is regenerated from the `codex/3d-completion-benchmark-g4` branch with `--require-image-to-mesh-providers`, so the Colab eval writes `image_to_mesh_provider_preflight.json` and fails before rows if `/content/TripoSG`, `/content/triposg-venv/bin/python`, or the TripoSG entrypoint is missing after setup. The branch-hosted archive is `1,623,701` bytes with SHA256 `ff68658498dd03015baf485520dd9e6a2f908bd95a3048e5c814bace4699e8c0`; the branch-hosted fetch notebook is `5,794` bytes with SHA256 `59742408ceab58fc701fbe699e0529b7731c4791caec8ff247bddbaca38f41f7`; and the fetch cell is `3,897` bytes with SHA256 `f4b7489e3a2aedb9f209a9455e43013a14e14ff779f924c217a57b38a9d3828e`. Open `https://colab.research.google.com/github/DrStrangel0ve/3dprintpic/blob/codex/3d-completion-benchmark-g4/colab_payloads/run_colab_triposg_bbox_tuning_eval_fetch_notebook.ipynb` on an actual G4 runtime for the next promotion-quality attempt.
