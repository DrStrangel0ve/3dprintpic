...
**Pinned TRELLIS.2 challenger.** The benchmark now has a separate `/content/trellis2-venv` setup path for the exact official TRELLIS.2 source/model revisions. It pins and validates the primary and transitive model snapshots, substitutes public exact-revision DINOv3 and BiRefNet snapshots for gated references, rewrites the pipeline manifest to local offline paths, uses the official `(x,y,z) -> (x,z,-y)` export convention, and requires an accepted sparse-attention backend. Generated Colab setup installs pinned `xformers`, verifies its CUDA attention operation, imports the official pipeline in the provider Python, records a provider preflight, and only then downloads the model snapshots or starts inference. A real G4 setup/inference smoke remains required before TRELLIS.2 can enter a quality comparison.
...
2. Keep pinned TripoSG as the full-mesh incumbent and preserve its adaptive scale-free face-density limit `9.95`.
3. Reprocess the five cached Pixal raw meshes with shape-preserving component filtering/closure, then compare them against the recorded coarse printable fallback and TripoSG with the same paired Chamfer/H95 gates.
4. Run the TRELLIS.2 setup-only G4 probe, then a one-row raw/repaired STL smoke if source, extension, attention, model-snapshot, and coordinate-frame checks all pass.
5. Score every depth-to-STL and direct-mesh output with the same watertightness, manifoldness, thickness, complexity, and Chamfer metrics; keep every source-mesh/source-bbox lane diagnostic-only.
6. Promote only a deployable backend that passes every per-sample STL gate, then expand beyond the current calibration slice.
7. Add a video/multiview learned challenger once the single-image STL scoring harness is stable.
