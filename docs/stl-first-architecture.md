...
Single-image mesh calibration update on July 10, 2026: `backend.benchmark.run_stl_first_smoke` now exposes `--mesh-target-bbox-source none|source|mirror|reference` and `--direct-mesh-reference-method`. This turns the recent mirror/reference bbox scaling probes into repeatable smoke-runner switches for TripoSR, TripoSG, Hunyuan3D, and other direct mesh commands. Bbox-calibrated candidates get explicit names such as `_stl_mirror_bbox_direct_mesh` or `_stl_reference_bbox_direct_mesh`, making G4 results comparable without hand-authoring one-off experiment JSON.
