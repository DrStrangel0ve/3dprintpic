...
def build_hunyuan3d_setup_prelude() -> str:
    return (
        ...
        "  rm -rf \"$HUNYUAN3D_DIR\"\n"
        "  git clone --filter=blob:none --depth 1 --sparse https://github.com/Tencent-Hunyuan/Hunyuan3D-2.1 \"$HUNYUAN3D_DIR\"\n"
        "  git -C \"$HUNYUAN3D_DIR\" sparse-checkout set hy3dshape/hy3dshape hy3dshape/configs\n"
        "elif [[ ! -d \"$HUNYUAN3D_DIR/hy3dshape/hy3dshape\" ]]; then\n"
        "  git -C \"$HUNYUAN3D_DIR\" sparse-checkout set hy3dshape/hy3dshape hy3dshape/configs\n"
        "fi\n"
        "python -m pip install -q virtualenv\n"
...
def build_colab_run_script(
    *,
    ...
    return (
        ...
        "PY\n"
        "set +e\n"
        f"{shell_join(command)} 2>&1 | tee \"$RUN_LOG\"\n"
